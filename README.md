...
The plugin uses managed Firebase Google sign-in and local Word/PDF/JSON exports. It does not need the Express server described below. See [plugin setup and deployment](figma-plugin/README.md). Google sign-in connects automatically through an encrypted, temporary Firebase Realtime Database handoff. No localhost or self-managed server is required.
