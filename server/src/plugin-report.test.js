...
test('plugin UI invalidates stale and failed selections before downloading', async () => {
  ...
  let retry
  let signedOut
  vm.runInNewContext(source, { setupAuth: ({ onSignedOut }) => { signedOut = onSignedOut; return { token: async () => 'test-token' } }, parent, window, setInterval: callback => { retry = callback; return 1 }, clearInterval() {}, document: { getElementById: id => elements.get(id) }, pdfmake: { addVirtualFileSystem() {} }, fonts: {}, Blob, importPlugin: () => ({ content: [], pages: [], source: { name: 'Test' } }) })
  ...
  elements.get('page-workspace').onclick()
  assert.equal(elements.get('workspace-page').hidden, true)
  message({ type: 'authenticated', email: 'test@example.com' })
  ...
  assert.equal(elements.get('pdf').disabled, true)
  assert.equal(elements.get('workspace-page').hidden, true)
  message({ type: 'guest-session' })
  assert.equal(elements.get('workspace-page').hidden, false)
  assert.equal(elements.get('export').disabled, false)
  assert.equal(elements.get('auth-status').textContent, 'Continuing as guest')
  signedOut()
  assert.equal(elements.get('workspace-page').hidden, true)
})
