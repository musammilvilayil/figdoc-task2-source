...
const code = await build({ entryPoints: [fileURLToPath(new URL('figma-plugin/figdoc/code.ts', root))], write: false, target: 'es2020' })
for (const dir of ['figma-plugin/', 'figma-plugin/figdoc/']) {
  ...
  manifest.networkAccess = { allowedDomains: ['none'], devAllowedDomains: ['http://localhost:5000'] }
